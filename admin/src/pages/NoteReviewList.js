...
const NoteReviewList = () => {
  ...
  const handleRejectSubmit = async (values) => {
    try {
      ...
      // 根据审核状态调用不同的API
      if (currentReview.status === 'ai_approved') {
        // 待人工复审状态，调用经理审核API
        await axios.put(
          `/reviews/${currentReview._id}/manager-approve`,
          {
            approved: false,
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small">
          ...
          {record.status === 'pending' && !['finance', 'hr'].includes(user?.role) && (
            <>
              <Popconfirm
                title="确认通过这条笔记审核？"
                onConfirm={async () => {
          ...
          {record.status === 'ai_approved' && ['mentor', 'manager', 'boss'].includes(user?.role) && (
            <>
              <Popconfirm
                ...
                onConfirm={async () => {
                  try {
                    const token = localStorage.getItem('token');
                    // 带教老师调用 /review 接口，主管和老板调用 /manager-approve 接口
                    if (user?.role === 'mentor') {
                      await axios.post(
                        `/reviews/${record._id}/review`,
                        { action: 'approve', comment: '人工复审通过' },
                        { headers: { Authorization: `Bearer ${token}` } }
                      );
                    } else {
                      await axios.put(
                    ...
                    console.error('人工复审失败:', error);
                    message.error(error.response?.data?.message || '人工复审失败');
                  }
                }}
              >
                <Button type="link" icon={<CheckOutlined />} style={{ color: 'green' }}>
          ...
          {record.status === 'mentor_approved' && user?.role === 'manager' && (
            <>
              <Popconfirm
                title="确认通过这条笔记审核？"
                onConfirm={async () => {
      ...
      >
        {currentReview && (
          <div>
            ...
            <p><strong>笔记作者：</strong>{currentReview.userNoteInfo?.author || '未填写'}</p>
            <p><strong>笔记标题：</strong>{currentReview.userNoteInfo?.title || '未填写'}</p>
